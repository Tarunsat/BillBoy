...
if %errorlevel% NEQ 0 (
    ...
    powershell -Command ^
        "& { $url='https://win.rustup.rs/x86_64'; $out='%TEMP%\rustup-init.exe'; Invoke-WebRequest -Uri $url -OutFile $out; Start-Process -FilePath $out -ArgumentList '-y','--default-toolchain','stable' -Wait; }"
    :: Add cargo to PATH for this session
    set "PATH=%USERPROFILE%\.cargo\bin;%PATH%"
    where cargo >nul 2>&1
    if !errorlevel! NEQ 0 (
        echo  [ERROR] Rust installation failed. Please install from https://rustup.rs and re-run.
        pause
        exit /b 1
    )
...
:: Ensure cargo is on PATH (in case already installed but not in current session)
set "PATH=%USERPROFILE%\.cargo\bin;%PATH%"
...
if %errorlevel% NEQ 0 (
    ...
    powershell -Command ^
        "& { $url='https://go.microsoft.com/fwlink/p/?LinkId=2124703'; $out='%TEMP%\MicrosoftEdgeWebview2Setup.exe'; Invoke-WebRequest -Uri $url -OutFile $out; Start-Process -FilePath $out -ArgumentList '/silent /install' -Wait; }"
...
:: Primary location: release bundle NSIS / MSI installer .exe
for /r "%SCRIPT_DIR%\src-tauri\target\release\bundle" %%f in (*.exe) do (
    set "EXE_PATH=%%f"
)

:: Fallback: bare release .exe (not inside deps or build subdirs)
if "%EXE_PATH%"=="" (
    for %%f in ("%SCRIPT_DIR%\src-tauri\target\release\*.exe") do (
        set "EXE_PATH=%%f"
    )
)

if "%EXE_PATH%"=="" (
    echo  [WARNING] Could not locate built .exe automatically.
    echo  Please create a desktop shortcut manually from:
    echo  %SCRIPT_DIR%\src-tauri\target\release\
    pause
    exit /b 0
)
...
powershell -NoProfile -Command ^
    "& { $ws = New-Object -ComObject WScript.Shell; $s = $ws.CreateShortcut('%SHORTCUT%'); $s.TargetPath = '%EXE_PATH%'; $s.WorkingDirectory = Split-Path '%EXE_PATH%'; $s.Description = 'BillBoy Billing App'; $s.Save(); }"
